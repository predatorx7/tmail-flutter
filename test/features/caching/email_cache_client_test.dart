...
void main() {
  ...
  group('[delete]', () {
    ...
    test('cache should not delete item which not in the list', () async {
      await emailCacheClient.insertItem(EmailFixtures.email1.id.toString(),
          EmailFixtures.email1.toEmailCache());

      await emailCacheClient.insertItem(EmailFixtures.email2.id.toString(),
  ...
  group('[update]', () {
    test('cache should update item when update to iem which not in cache',
        () async {
      await emailCacheClient.updateItem(EmailFixtures.email1.id.toString(),
          EmailFixtures.email1.toEmailCache());
