...
abstract class BaseLoginView extends GetWidget<LoginController> {
  ...
  Widget buildLoginMessage(
      BuildContext context, Either<Failure, Success> viewState) {
    return Padding(
        padding:
            const EdgeInsets.only(top: 11, bottom: 36, left: 58, right: 58),
        child: SizedBox(
            width: responsiveUtils.getWidthLoginTextField(context),
            child: Text(
              viewState.fold((failure) {
                ...
                } else if (controller.loginFormType.value ==
