...
class ThreadRepositoryImpl extends ThreadRepository {
  ...
  Stream<EmailsResponse> getAllEmail(
    ...
    if (!localEmailResponse.hasEmails() ||
      ...
      if (_isApproveFilterOption(
          emailFilter?.filterOption, networkEmailResponse.emailList)) {
        _getFirstPage(
          session,
          accountId,
          sort: sort,
    ...
    if (networkEmailResponse != null) {
      await _updateEmailCache(accountId, session.username,
    ...
    if (localEmailResponse.hasState()) {
      ...
      await _synchronizeCacheWithChanges(
          session, accountId, localEmailResponse.state!,
          propertiesCreated: propertiesCreated,
      ...
      if (networkEmailResponse != null) {
        ...
        if (networkEmailResponse.state != null) {
          await _updateState(
              accountId, session.username, networkEmailResponse.state!);
        }
      }
    }
  ...
  bool _isApproveFilterOption(
      FilterMessageOption? filterOption, List<Email>? listEmailResponse) {
  ...
  Future<EmailsResponse> _getFirstPage(
    ...
    final networkEmailResponse =
        await mapDataSource[DataSourceType.network]!.getAllEmail(
      session,
      accountId,
  ...
  Future<List<Email>?> _combineEmailCache(
      ...
      List<Email>? emailCacheList}) async {
    if (emailUpdated != null && emailUpdated.isNotEmpty) {
      ...
      final newEmailUpdated = emailUpdated
          ...
          .map((tuple) =>
              tuple.value2!.combineEmail(tuple.value1, updatedProperties))
          .toList();
  ...
  dartz.Tuple2<Email, Email?> _combineUpdatedWithEmailInCache(
      Email updatedEmail, List<Email>? emailCacheList) {
  ...
  Future<void> _updateEmailCache(AccountId accountId, UserName userName,
      {List<Email>? newUpdated,
      List<Email>? newCreated,
      List<EmailId>? newDestroyed}) async {
    await mapDataSource[DataSourceType.local]!.update(accountId, userName,
        updated: newUpdated, created: newCreated, destroyed: newDestroyed);
  }

  Future<void> _updateState(
      AccountId accountId, UserName userName, State newState) async {
    log('ThreadRepositoryImpl::_updateState(): [MAIL] $newState');
    await stateDataSource.saveState(
        accountId, userName, newState.toStateCache(StateType.email));
  }
  ...
  Stream<EmailsResponse> loadMoreEmails(GetEmailRequest emailRequest) async* {
    ...
    await _updateEmailCache(
        emailRequest.accountId, emailRequest.session.username,
  ...
  Future<EmailsResponse> _getAllEmailsWithoutLastEmailId(
    ...
    final emailResponse = await mapDataSource[DataSourceType.network]!
        .getAllEmail(emailRequest.session, emailRequest.accountId,
  ...
  @override
  Future<List<Email>> searchEmails(Session session, AccountId accountId,
      {UnsignedInt? limit,
      Set<Comparator>? sort,
      Filter? filter,
  ...
  Future<void> _synchronizeCacheWithChanges(
    ...
    if (emailChangeResponse != null) {
      ...
      if (emailChangeResponse.newStateEmail != null) {
        await _updateState(
            accountId, session.username, emailChangeResponse.newStateEmail!);
      }
    }
  }
  ...
  Future<List<EmailId>> emptySpamFolder(
      Session session, AccountId accountId, MailboxId spamMailboxId) {
    return mapDataSource[DataSourceType.network]!.emptyMailboxFolder(
      session,
      accountId,
      spamMailboxId,
