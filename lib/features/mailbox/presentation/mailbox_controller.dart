...
class MailboxController extends BaseMailboxController
  ...
  @override
  void onReady() {
    _openMailboxEventController.stream
        .debounceTime(const Duration(milliseconds: 500))
        .listen((event) {
      _handleOpenMailbox(event.buildContext, event.presentationMailbox);
    });
    _initCollapseMailboxCategories();
    mailboxListScrollController
        .addListener(_mailboxListScrollControllerListener);
    super.onReady();
  }
  ...
  void handleSuccessViewState(Success success) {
    ...
    if (success is GetAllMailboxSuccess) {
      ...
      _deleteMultipleMailboxSuccess(
          success.listMailboxIdDeleted, success.currentMailboxState);
  ...
  void _registerObxStreamListener() {
    ...
    ever<Map<String, String?>?>(mailboxDashBoardController.routerParameters,
  ...
  void _setMapMailbox() {
    final mapDefaultMailboxIdByRole = {
      for (var mailboxNode in defaultMailboxTree.value.root.childrenItems ??
    ...
    mailboxDashBoardController
        .setMapDefaultMailboxIdByRole(mapDefaultMailboxIdByRole);
  ...
  void _setOutboxMailbox() {
    try {
      ...
      if (outboxMailboxIdByRole == null) {
        final outboxMailboxByName = allMailboxes.firstWhere((mailbox) =>
            mailbox.name?.toLowerCase() ==
            PresentationMailbox.lowerCaseOutboxMailboxName);
  ...
  PresentationMailbox? _getCurrentSelectedMailbox() {
    ...
    if (mailboxCurrent != null) {
      if (mailboxCurrent.hasRole()) {
        return mapDefaultPresentationMailboxByRole
                .containsKey(mailboxCurrent.role)
      ...
      if (mapDefaultPresentationMailboxByRole
          .containsKey(PresentationMailbox.roleInbox)) {
        return mapDefaultPresentationMailboxByRole[
            PresentationMailbox.roleInbox];
  ...
  void _handleDataFromNavigationRouter() {
    ...
    if (isHasDataFromRoute) {
      if (mailboxIdFromNavigationRouter != null) {
        ...
        mailboxDashBoardController
            .dispatchAction(SelectEmailByIdAction(navigationRouter!));
        _clearNavigationRouter();
      } else if (searchQueryFromNavigationRouter != null &&
          searchQueryFromNavigationRouter?.value.isNotEmpty == true) {
        mailboxDashBoardController
            .dispatchAction(SearchEmailByQueryAction(navigationRouter!));
        _clearNavigationRouter();
      } else {
        _clearNavigationRouter();
        _selectSelectedMailboxDefault();
  ...
  NavigationRouter? get navigationRouter =>
      mailboxDashBoardController.navigationRouter;
  ...
  SearchQuery? get searchQueryFromNavigationRouter =>
      navigationRouter?.searchQuery;
  ...
  void _selectMailboxFromRouter() {
    final matchedMailboxNode =
        findMailboxNodeById(mailboxIdFromNavigationRouter!);
  ...
  void _handleOpenMailbox(
      BuildContext context, PresentationMailbox presentationMailboxSelected) {
  ...
  void _disableAllSearchEmail() {
    mailboxDashBoardController
        .dispatchAction(ClearAllFieldOfAdvancedSearchAction());
    mailboxDashBoardController.searchController.disableAllSearchEmail();
  }

  void openMailbox(
      BuildContext context, PresentationMailbox presentationMailboxSelected) {
    _openMailboxEventController
        .add(OpenMailboxViewEvent(context, presentationMailboxSelected));
  }
    ...
    if (session != null && accountId != null) {
      ...
      final result = PlatformInfo.isWeb
          ? await DialogRouter.pushGeneralDialog(
              routeName: AppRoutes.mailboxCreator, arguments: arguments)
      ...
      if (result != null && result is NewMailboxArguments) {
        ...
        _createNewMailboxAction(
            session,
            accountId,
            CreateNewMailboxRequest(generateCreateId, result.newName,
                parentId: result.mailboxLocation?.id));
      }
    }
  }

  void _createNewMailboxAction(Session session, AccountId accountId,
  ...
  void _createNewMailboxSuccess(CreateNewMailboxSuccess success) {
    if (currentOverlayContext != null && currentContext != null) {
      _appToast.showToastSuccessMessage(
          ...
          AppLocalizations.of(currentContext!).createFolderSuccessfullyMessage(
              success.newMailbox.name?.name ?? ''),
  ...
  void pressMailboxSelectionAction(BuildContext context, MailboxActions actions,
    ...
    switch (actions) {
      case MailboxActions.delete:
        _openConfirmationDialogDeleteMultipleMailboxAction(
            context, selectedMailboxList);
        break;
      case MailboxActions.rename:
        openDialogRenameMailboxAction(
            context, selectedMailboxList.first, _responsiveUtils,
            onRenameMailboxAction: _renameMailboxAction);
        break;
      case MailboxActions.markAsRead:
        markAsReadMailboxAction(
            context, selectedMailboxList.first, mailboxDashBoardController,
        ...
        moveMailboxAction(
            ...
            onMovingMailboxAction: (mailboxSelected, destinationMailbox) =>
                _invokeMovingMailboxAction(
                    context, mailboxSelected, destinationMailbox));
        break;
      default:
        break;
    }
  ...
  void _deleteMailboxAction(PresentationMailbox presentationMailbox) {
    ...
    if (session != null && accountId != null) {
      ...
      consumeState(_deleteMultipleMailboxInteractor.execute(
          session, accountId, mapDescendantIds, listMailboxId));
    } else {
  ...
  void _deleteMultipleMailboxSuccess(
      List<MailboxId> listMailboxIdDeleted, jmap.State? currentMailboxState) {
    if (currentOverlayContext != null && currentContext != null) {
  ...
  void _openConfirmationDialogDeleteMultipleMailboxAction(
      BuildContext context, List<PresentationMailbox> selectedMailboxList) {
  ...
  void _deleteMultipleMailboxAction(
      List<PresentationMailbox> selectedMailboxList) {
    final accountId = mailboxDashBoardController.accountId.value;
    final session = mailboxDashBoardController.sessionCurrent;
  ...
  void _renameMailboxAction(
      PresentationMailbox presentationMailbox, MailboxName newMailboxName) {
    final accountId = mailboxDashBoardController.accountId.value;
    final session = mailboxDashBoardController.sessionCurrent;

    if (session != null && accountId != null) {
      consumeState(_renameMailboxInteractor.execute(session, accountId,
  ...
  void _handleMovingMailbox(
    ...
    consumeState(_moveMailboxInteractor.execute(
        session,
        accountId,
        MoveMailboxRequest(mailboxSelected.id, moveAction,
      ...
      _appToast.showToastMessage(
          ...
          AppLocalizations.of(currentContext!).moved_to_mailbox(
              success.destinationMailboxDisplayName ??
                  AppLocalizations.of(currentContext!).allMailboxes),
          actionName: AppLocalizations.of(currentContext!).undo,
          onActionClick: () {
        _undoMovingMailbox(MoveMailboxRequest(
            success.mailboxIdSelected, MoveAction.undo,
  ...
  void toggleMailboxCategories(MailboxCategories categories) async {
    switch (categories) {
      ...
      case MailboxCategories.teamMailboxes:
        ...
        mailboxCategoriesExpandMode.refresh();
        if (personalMailboxTree.value.root.hasChildren() &&
            mailboxCategoriesExpandMode.value.teamMailboxes ==
                ExpandMode.COLLAPSE) {
          _triggerToggleMailboxCategories();
        }
        break;
      case MailboxCategories.appGrid:
        final currentExpandMode = mailboxDashBoardController
            .appGridDashboardController.appDashboardExpandMode.value;
    ...
    switch (actions) {
      case MailboxActions.delete:
        openConfirmationDialogDeleteMailboxAction(
            context, _responsiveUtils, _imagePaths, mailbox,
            onDeleteMailboxAction: _deleteMailboxAction);
        break;
      case MailboxActions.rename:
        openDialogRenameMailboxAction(context, mailbox, _responsiveUtils,
            onRenameMailboxAction: _renameMailboxAction);
        break;
      case MailboxActions.move:
        moveMailboxAction(context, mailbox, mailboxDashBoardController,
            onMovingMailboxAction: (mailboxSelected, destinationMailbox) =>
                _invokeMovingMailboxAction(
                    context, mailboxSelected, destinationMailbox));
        break;
      case MailboxActions.markAsRead:
        markAsReadMailboxAction(context, mailbox, mailboxDashBoardController,
            onCallbackAction: closeMailboxScreen);
        break;
  ...
  void _invokeMovingMailboxAction(
    ...
    if (session != null && accountId != null) {
      _handleMovingMailbox(
          context, session, accountId, MoveAction.moving, mailboxSelected,
  ...
  void stopAutoScroll() {
    mailboxListScrollController.animateTo(mailboxListScrollController.offset,
  ...
  void _handleRefreshChangesAllMailboxSuccess(
      RefreshChangesAllMailboxSuccess success) async {
    currentMailboxState = success.currentMailboxState;
    final listMailboxDisplayed =
        success.mailboxList.listSubscribedMailboxesAndDefaultMailboxes;
  ...
  void _unsubscribeMailboxAction(MailboxId mailboxId) {
    ...
    final session = mailboxDashBoardController.sessionCurrent;
    if (session != null && accountId != null) {
      final subscribeRequest = generateSubscribeRequest(mailboxId,
          MailboxSubscribeState.disabled, MailboxSubscribeAction.unSubscribe);
  ...
  void _closeEmailViewIfMailboxDisabledOrNotExist(
    ...
    final mailboxContain = selectedEmail!
  ...
  void _undoUnsubscribeMailboxAction(MailboxId mailboxIdSubscribed,
      {List<MailboxId>? listDescendantMailboxIds}) {
    final accountId = mailboxDashBoardController.accountId.value;
    final session = mailboxDashBoardController.sessionCurrent;
    ...
    if (session != null && accountId != null) {
      ...
      if (listDescendantMailboxIds != null) {
        ...
        subscribeRequest = SubscribeMailboxRequest(mailboxIdSubscribed,
            MailboxSubscribeState.enabled, MailboxSubscribeAction.undo);
      }
  ...
  void _autoScrollToTopMailboxList() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mailboxListScrollController.hasClients) {
        mailboxListScrollController.animateTo(0,
  ...
  void emptyMailboxAction(
      BuildContext context, PresentationMailbox presentationMailbox) {
