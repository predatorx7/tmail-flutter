...
class ContactInputTagItem extends StatelessWidget {
  ...
  const ContactInputTagItem(this.contact,
      {Key? key,
  ...
  Widget build(BuildContext context) {
    ...
    final itemChild = Chip(
      ...
      avatar: contact.displayName.isNotEmpty
          ? GradientColorAvatarIcon(contact.avatarColors,
              iconSize: 20, labelFontSize: 11, label: contact.labelAvatar)
          : null,
      labelPadding: const EdgeInsets.symmetric(horizontal: 8),
      padding: DirectionUtils.isDirectionRTLByHasAnyRtl(contact.asString())
          ? EdgeInsets.zero
          : const EdgeInsets.all(6),
      label: Text(contact.asString(),
          maxLines: 1,
      ...
      deleteIcon: deleteContactCallbackAction != null
          ? SvgPicture.asset(imagePaths.icClose,
              width: 20,
              height: 20,
      ...
      shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(20), side: _getTagBorderSide()),
      onDeleted: () => deleteContactCallbackAction?.call(contact),
    );

    if (PlatformInfo.isWeb) {
      return Padding(padding: const EdgeInsets.only(top: 10), child: itemChild);
    } else {
      return Padding(padding: const EdgeInsets.only(top: 8), child: itemChild);
    }
  }

  bool _isValidEmailAddress(String value) =>
      value.isEmail || AppUtils.isEmailLocalhost(value);
