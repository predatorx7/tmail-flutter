...
mixin FilterEmailPopupMenuMixin {
    ...
    return [
      if (!isSearchEmailRunning) ...[
        PopupMenuItem(
            ...
            child: _filterEmailAction(context, optionSelected,
                FilterMessageOption.attachments, onCallBack)),
        const PopupMenuDivider(height: 0.5)
      ],
      PopupMenuItem(
          padding: EdgeInsets.zero,
          child: _filterEmailAction(
              context, optionSelected, FilterMessageOption.unread, onCallBack)),
      const PopupMenuDivider(height: 0.5),
      PopupMenuItem(
          padding: EdgeInsets.zero,
          child: _filterEmailAction(context, optionSelected,
              FilterMessageOption.starred, onCallBack)),
    ];
  }
  ...
  Widget _filterEmailAction(
    ...
    return InkWell(
        onTap: () => onCallBack
            ?.call(option == optionSelected ? FilterMessageOption.all : option),
        child: Padding(
