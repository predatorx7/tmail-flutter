...
typedef OnDownloadAttachmentActionCallback = Function(
          ...
          Row(
            children: [
              Expanded(
                ...
                SvgPicture.asset(_imagePaths.icAttachment,
                    width: EmailAttachmentsStyles.headerIconSize,
                    height: EmailAttachmentsStyles.headerIconSize,
                    colorFilter:
                        EmailAttachmentsStyles.headerIconColor.asFilter(),
                    fit: BoxFit.fill),
                const SizedBox(width: EmailAttachmentsStyles.headerSpace),
                Expanded(
                    child: Text(
              ...
              if (widget.attachments.length > 2)
                TMailButtonWidget(
                  ...
                  onTapActionCallback: () =>
                      setState(() => _isDisplayAll = !_isDisplayAll),
                )
            ],
          ),
          ...
          if (_isDisplayAll)
            Wrap(
                ...
                children: widget.attachments
                    .map((attachment) => AttachmentFileTileBuilder(attachment,
                        onDownloadAttachmentFileActionClick: (attachment) =>
                            widget.onDownloadAttachmentActionCallback
                                ?.call(context, attachment)))
                    .toList())
          else
            SizedBox(
