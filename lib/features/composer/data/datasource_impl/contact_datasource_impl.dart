  ...
  Future<List<Contact>> getContactSuggestions(
    ...
    return Future.sync(() async {
      if (autoCompletePattern.word.isEmpty) {
        ...
        final suggestedList =
            await contact_service.ContactsService.getContactsByEmailOrName(
  ...
  List<DeviceContact> _toDeviceContact(contact_service.Contact contact) {
    if (contact.emails != null) {
      return contact.emails!
          .where(
              (email) => email.value != null && GetUtils.isEmail(email.value!))
          .map((email) =>
              DeviceContact(contact.displayName ?? '', email.value ?? ''))
          .toList();
    }
    return <DeviceContact>[];
  }
