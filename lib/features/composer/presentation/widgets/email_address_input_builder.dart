...
typedef OnUpdateListEmailAddressAction = void Function(
    PrefixEmailAddress, List<EmailAddress>);
...
typedef OnFocusEmailAddressChangeAction = void Function(
    PrefixEmailAddress, bool);
...
class EmailAddressInputBuilder {
  ...
  void addOnUpdateListEmailAddressAction(
  ...
  void addOnSuggestionEmailAddress(
      OnSuggestionEmailAddress onSuggestionEmailAddress) {
    _onSuggestionEmailAddress = onSuggestionEmailAddress;
  }

  void addOnAddEmailAddressTypeAction(
      OnAddEmailAddressTypeAction onAddEmailAddressTypeAction) {
    _onAddEmailAddressTypeAction = onAddEmailAddressTypeAction;
  }

  void addOnDeleteEmailAddressTypeAction(
      OnDeleteEmailAddressTypeAction onDeleteEmailAddressTypeAction) {
    _onDeleteEmailAddressTypeAction = onDeleteEmailAddressTypeAction;
  }

  void addOnShowFullListEmailAddressAction(
      OnShowFullListEmailAddressAction onShowFullListEmailAddressAction) {
    _onShowFullListEmailAddressAction = onShowFullListEmailAddressAction;
  }

  void addOnFocusEmailAddressChangeAction(
      OnFocusEmailAddressChangeAction onFocusEmailAddressChangeAction) {
    _onFocusEmailAddressChangeAction = onFocusEmailAddressChangeAction;
  }

  void addOnFocusNextAddressAction(
      OnFocusNextAddressAction? onFocusNextAddressAction) {
      ...
      child: Row(children: [
        Text('${_prefixEmailAddress.asName(_context)}:',
            style: const TextStyle(
                fontSize: 15, color: AppColor.colorHintEmailAddressInput)),
        ...
        if (_prefixEmailAddress == PrefixEmailAddress.to)
          Row(children: [
            if (!_listEmailAddressType.contains(PrefixEmailAddress.cc))
              buildTextIcon(
                  ...
                  onTap: () => _onAddEmailAddressTypeAction
                      ?.call(PrefixEmailAddress.cc)),
          ...
          buildIconWeb(
              ...
              onTap: () =>
                  _onDeleteEmailAddressTypeAction?.call(_prefixEmailAddress))
      ]),
    );
  }
  ...
  Widget _buildTagEditor() {
    return StatefulBuilder(
      ...
      final newListEmailAddress =
          _isCollapse ? listEmailAddress.sublist(0, 1) : listEmailAddress;
      return FocusScope(
          child: Focus(
              onFocusChange: (focus) => _onFocusEmailAddressChangeAction?.call(
                  _prefixEmailAddress, focus),
              ...
              child: TagEditor<SuggestionEmailAddress>(
                ...
                textStyle: const TextStyle(
                    color: AppColor.colorEmailAddress,
                    fontSize: 14,
                ...
                onDeleteTagAction: () =>
                    _handleDeleteTagAction(setState, context),
                onSelectOptionAction: (item) {
                  if (!_isDuplicatedRecipient(item.emailAddress.emailAddress)) {
                    ...
                    _onUpdateListEmailAddressAction?.call(
                        _prefixEmailAddress, listEmailAddress);
                  }
                },
                onSubmitted: (value) {
                  ...
                  if (!_isDuplicatedRecipient(textTrim)) {
                    setState(() =>
                        listEmailAddress.add(EmailAddress(null, textTrim)));
                    _onUpdateListEmailAddressAction?.call(
                        _prefixEmailAddress, listEmailAddress);
                  }
                },
                  ...
                  return Stack(
                      ...
                      children: [
                        Padding(
                            ...
                            child: InkWell(
                                onTap: () => _isCollapse
                                    ? _onShowFullListEmailAddressAction
                                        ?.call(_prefixEmailAddress)
                                    : null,
                                child: Chip(
                                  padding:
                                      DirectionUtils.isDirectionRTLByLanguage(
                                              context)
                                          ? EdgeInsets.zero
                                          : null,
                                  labelPadding: EdgeInsetsDirectional.symmetric(
                                      horizontal: 12,
                                      vertical: DirectionUtils
                                              .isDirectionRTLByHasAnyRtl(
                                                  newListEmailAddress[index]
                                                      .asString())
                                          ? 0
                                          : 2),
                                  label: Text(
                                  ...
                                  backgroundColor: _getTagBackgroundColor(
                                      listEmailAddress[index], isLastEmail),
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(10),
                                    side: _getTagBorderSide(
                                        listEmailAddress[index], isLastEmail),
                                  ),
                                  avatar: newListEmailAddress[index]
                                          .displayName
                                          .isNotEmpty
                                      ? CircleAvatar(
                                          backgroundColor:
                        ...
                        if (_isCollapse)
                          _buildCounter(
                              context,
                              listEmailAddress.length -
                                  newListEmailAddress.length),
                      ]);
                },
                onTagChanged: (value) {
                ...
                suggestionBuilder: (context,
                  ...
                  switch (suggestionEmailAddress.state) {
                    case SuggestionEmailState.duplicated:
                      return _buildExistedSuggestionItem(setState, context,
                          suggestionEmailAddress.emailAddress, suggestionValid);
                    default:
  ...
  Widget _buildCounter(BuildContext context, int count) {
    return Padding(
      padding: const EdgeInsetsDirectional.only(
          start: 8, top: PlatformInfo.isWeb ? 8 : 0),
      child: InkWell(
        onTap: () =>
        ...
        child: Chip(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: const BorderSide(
                width: 0, color: AppColor.colorEmailAddressTag),
          ),
        ),
      ),
    );
  ...
  Widget _buildExistedSuggestionItem(StateSetter setState, BuildContext context,
      EmailAddress emailAddress, String? suggestionValid) {
    return Container(
        padding: const EdgeInsets.all(8.0),
        decoration: const BoxDecoration(
            borderRadius:
                BorderRadius.all(Radius.circular(_suggestionBoxRadius))),
        child: Container(
            decoration: const BoxDecoration(
            ...
            child: ListTile(
              ...
              subtitle:
                  _buildSubtitleSuggestionItem(emailAddress, suggestionValid),
  ...
  Widget _buildTitleSuggestionItem(
      EmailAddress emailAddress, String? suggestionValid) {
  ...
  Widget? _buildSubtitleSuggestionItem(
      EmailAddress emailAddress, String? suggestionValid) {
  ...
  FutureOr<List<SuggestionEmailAddress>> _findSuggestions(String query) async {
    ...
    if (processedQuery.length >= AppConstants.limitCharToStartSearch &&
        _onSuggestionEmailAddress != null) {
      tmailSuggestion.addAll((await _onSuggestionEmailAddress!(processedQuery))
          .map((emailAddress) =>
              _toSuggestionEmailAddress(emailAddress, listEmailAddress)));
    }

    tmailSuggestion.addAll(
        _matchedSuggestionEmailAddress(processedQuery, listEmailAddress));
  ...
  bool _isEmailAddressValid(String emailAddress) =>
  ...
  Iterable<SuggestionEmailAddress> _matchedSuggestionEmailAddress(
      String query, List<EmailAddress> addedEmailAddress) {
    return addedEmailAddress
        .where((addedMail) => addedMail.emailAddress.contains(query))
        .map((emailAddress) => SuggestionEmailAddress(emailAddress,
            state: SuggestionEmailState.duplicated));
  }
