...
class SanitizeAutolinkFilter {
  ...
  final _linkifyOption = const LinkifyOptions(
      humanize: true, looseUrl: true, defaultToHttps: true, removeWww: true);
  final _linkifier = <Linkifier>[const EmailLinkifier(), const UrlLinkifier()];
  ...
  String process(String inputText) {
    ...
    final elements =
        linkify(inputText, options: _linkifyOption, linkifiers: _linkifier);
    ...
    for (var element in elements) {
      if (element is TextElement) {
        ...
        final emailLinkTag =
            _buildEmailLinkTag(mailToLink: element.url, value: element.text);
        htmlTextBuffer.write(emailLinkTag);
      } else if (element is UrlElement) {
        final urlLinkTag =
            _buildUrlLinkTag(urlLink: element.url, value: element.text);
